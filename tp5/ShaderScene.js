...
export class ShaderScene extends CGFscene {
	...
	init(application) {
		...
		this.testShaders = [
			...
			new CGFshader(this.gl, "shaders/exercice1.vert", "shaders/exercice1.frag"),
		...
		this.testShaders[9].setUniformsValues({ scaleFactor: 0 });

		this.testShaders[11].setUniformsValues({ scaleFactor: 0 });
	...
	update(t) {
		...
		if (this.selectedExampleShader == 9 || this.selectedExampleShader == 6 || this.selectedExampleShader == 11){
			...
			this.testShaders[11].setUniformsValues({ scaleFactor: this.scaleFactor });
